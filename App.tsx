          ...
          <div className="w-80 bg-slate-950 p-6 overflow-y-auto">
            ...
            <div className="bg-slate-900 rounded-xl p-4 border border-slate-800 mb-6 shadow-lg">
              ...
              <div className="text-center py-4">
                ...
                <button
                  onClick={toggleTimer}
                  className={`w-full py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${timerActive
...
const App: React.FC = () => {
  ...
  const handleTaskUpdate = async (originalTask: Task, updates: any) => {
    ...
    setTasks(prev => prev.map(t => t.id === originalTask.id ? {
      ...t, ...updates,
  ...
  const downloadCSV = () => {
    ...
    const rows = tasks.map(t => {
      ...
      const customs = sortedCustomFields.map(cf => {
        ...
        // The value is already resolved to a string by ClickUpService.resolveCustomFieldValue
        return field.value;
      });
